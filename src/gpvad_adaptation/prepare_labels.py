...
SAMPLING_RATE = 22050
HOP_LEN = int(SAMPLING_RATE * 20 / 1000) 
...
arr_counter1 = np.zeros(10)
fname_bad = []

def sigmoid(x, alpha=1):
    return 1 / (1 + np.exp(-alpha * x))

def generate_soft_labels(probability, threshold):
    alpha = 1
    return sigmoid(probability - threshold, alpha)


def LinearSoftPool(time_decision):
    return (time_decision**2).sum(0) / time_decision.sum(0)
...
def main():
    ...
    with torch.no_grad(), tqdm(total=len(dataloader)) as pbar, open("labels.csv",'r') as csvfile:
        ...
        for row in csvwr:
            if line_count == 0:
                line_count+=1
                continue
            labels.append(np.array(ast.literal_eval(row[1])))
            names.append(row[0])
    ...
    with torch.no_grad(), File(args.hdf5output, 'w') as store, tqdm(
        ...
        for batch in dataloader:
            ...
            fname = fname[0]
            index = names.index("signals/"+fname)
            ...
            not_ptt_clip_pred = LinearSoftPool(labels[index])
            ptt_clip_pred = LinearSoftPool(inversed)

            #func(sum(labels[index]), sum(ptt_time_pred))
            ...
            inversed = 1 - labels[index]
            
            store[f'{fname}/ptt'] = labels[index] #ptt_time_pred#
            store[f'{fname}/notptt'] = inversed #not_ptt_time_pred#
            store[f'{fname}/clipptt'] = ptt_clip_pred
            store[f'{fname}/clipnotptt'] = not_ptt_clip_pred
