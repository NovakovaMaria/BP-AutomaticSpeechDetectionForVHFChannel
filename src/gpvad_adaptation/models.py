...
# model CRNN edited by Maria Novakova for Bachelor Thesis

def crnn(inputdim=64, outputdim=527, pretrained_from='vad.pth'):
...
def cnn10(inputdim=64, outputdim=527, pretrained_from='vad.pth'):
...
class LinearSoftPool(nn.Module):
    ...
    def forward(self, logits, time_decision):
        #print(time_decision.sum(1), (time_decision**2).sum(1))
...
class CRNN(nn.Module):
    def __init__(self, inputdim, outputdim, **kwargs):
        ...
        self.temp_pool = parse_poolingfunction(kwargs.get('temppool', 'linear'),
                                               inputdim=256,
                                               outputdim=outputdim)
        self.outputlayer = nn.Linear(256, outputdim) # toto pretrenovat?

        self.outputlayer1 = nn.Linear(256, outputdim) # PTT output

        #self.features.apply(init_weights)
        #self.outputlayer.apply(init_weights)
        self.outputlayer1.apply(init_weights)
    ...
    def forward(self, x, upsample=True):
        ...
        decision_time1 = torch.sigmoid(self.outputlayer1(x)).clamp(1e-7, 1.)
        ...
        if upsample:
            ...
            decision_time1 = torch.nn.functional.interpolate(
                decision_time1.transpose(1, 2),
                time,
                mode='linear',
                align_corners=False).transpose(1, 2)
            
        decision = self.temp_pool(x, decision_time).clamp(1e-7, 1.).squeeze(1)
        decision1 = self.temp_pool(x, decision_time1).clamp(1e-7, 1.).squeeze(1)

        return decision, decision_time, decision1, decision_time1
