...
class TrainHDF5Dataset(tdata.Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        ptt_target = self._labelcache[labelh5file][f"{fname}/ptt"][()]
        non_ptt_target = self._labelcache[labelh5file][f"{fname}/notptt"][()]

        #print(non_ptt_target, ptt_target, speech_target, noise_target)

        speech_clip_target = self._labelcache[labelh5file][
            f"{fname}/clipspeech"][()]
        noise_clip_target = self._labelcache[labelh5file][
            f"{fname}/clipnoise"][()]
        ptt_clip_target = self._labelcache[labelh5file][
            f"{fname}/clipptt"][()]
        not_ptt_clip_target = self._labelcache[labelh5file][
            f"{fname}/clipnotptt"][()]
        ...
        ptt_target_clip = torch.tensor((not_ptt_clip_target, ptt_clip_target))
        ...
        #print(non_ptt_target.shape, ptt_target.shape)

        ptt_target_time = torch.as_tensor(
            np.stack((non_ptt_target, ptt_target), axis=-1)).float()
        
        if self._transform:
            data = self._transform(data)
    
        return data, target_time, target_clip, ptt_target_time, ptt_target_clip, fname
...
class HDF5Dataset(tdata.Dataset):
    ...
    def __getitem__(self, index):
        ...
        ptt_target = self.label_dataset[f"{fname}/ptt"][()]
        non_ptt_target = self.label_dataset[f"{fname}/notptt"][()]

        speech_clip_target = self.label_dataset[f"{fname}/clipspeech"][()]
        noise_clip_target = self.label_dataset[f"{fname}/clipnoise"][()]
        ptt_clip_target = self.label_dataset[f"{fname}/clipptt"][()]
        not_ptt_clip_target = self.label_dataset[f"{fname}/clipnotptt"][()]
...
def sequential_collate(batches):
    # sort length wise
    data, targets_time, targets_clip, ptt_target_time, ptt_target_clip, fnames = zip(*batches)
    ...
    ptt_target_time, lengths_tar = pad(ptt_target_time, padding_value=0)
    assert lengths_data.shape == lengths_tar.shape

    targets_clip = torch.stack(targets_clip)
    ptt_target_clip = torch.stack(ptt_target_clip)
    return data, targets_time, targets_clip, ptt_target_time, ptt_target_clip, fnames, lengths_tar
